...
impl NonFungibleTokenCore for Contract {
    ...
    fn nft_approve(&mut self, token_id: TokenId, account_id: AccountId, msg: Option<String>) {
        ...
        */
        ...
        if let Some(msg) = msg {
            // if msg contain `delegate` -> delegate can update metadata
            // callback by nft_delegate smart contract
            // format: delegate@<new metadata>

            let splitted_string: Vec<&str> = msg.as_str().split('@').collect();
            if splitted_string[0] == "delegate" {
                let new_metadata_string = splitted_string[1];
                let json_result: TokenMetadata =
                    near_sdk::serde_json::from_str(&new_metadata_string)
                        .expect("Not valid metadata args");
                ext_non_fungible_approval_receiver::ext(env::current_account_id())
                    .nft_on_approve(token_id.clone(), token.owner_id, approval_id, msg.clone())
                    .then(
                        ext_self::ext(env::current_account_id())
                            .with_static_gas(Gas(10_000_000_000_000))
                            .update_metadata_callback(token_id, json_result),
                    );
            } else {
